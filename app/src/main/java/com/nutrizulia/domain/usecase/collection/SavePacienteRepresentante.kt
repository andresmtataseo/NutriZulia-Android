...
class SavePacienteRepresentante @Inject constructor(
    private val repository: PacienteRepresentanteRepository
) {
    suspend operator fun invoke(pacienteRepresentante: PacienteRepresentante) {
        repository.upsert(pacienteRepresentante)
    }
}