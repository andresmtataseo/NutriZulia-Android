...
class RegistrarCitaViewModel @Inject constructor(
    ...
    val tiposActividades: MutableLiveData<List<TipoActividad>> = _tiposActividades
    private val _especialidades = MutableLiveData<List<Especialidad>>()
    val especialidades: MutableLiveData<List<Especialidad>> = _especialidades
    private val _tiposConsultas = MutableLiveData<List<TipoConsulta>>()
    val tiposConsultas: MutableLiveData<List<TipoConsulta>> = _tiposConsultas

    private val _errores = MutableLiveData<Map<String, String>>()
    val errores: MutableLiveData<Map<String, String>> = _errores
    ...
    private fun validarConsulta(consulta: Consulta): Map<String, String> {
        ...
        if (consulta.tipoConsulta == null) erroresActuales["tipoConsulta"] = "El tipo de consulta es obligatorio."

        if (consulta.fechaHoraProgramada == null) {
            erroresActuales["fechaProgramada"] = "La fecha de la cita no puede estar vacía."
        }
    ...
    fun registrarConsulta(consulta: Consulta) {
        ...
        viewModelScope.launch {
            ...
            sessionManager.currentInstitutionIdFlow.firstOrNull()?.let { institutionId ->
                ...
                consulta.usuarioInstitucionId = id
            } ?: run {
            ...
            val result = saveConsulta(consulta)
            if (result > 0) {
